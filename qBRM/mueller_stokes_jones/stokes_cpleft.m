function V = stokes_cpleft(varargin)
    %function V = stokes_cpleft(varargin)
    %Return the Stokes vector for left-turn circularly polarized light.
    %
    %  Syntax:
    %     V = stokes_cpleft()
    %     V = stokes_cpleft(p)
    ...
    %
    %  Description:
    %     V = stokes_cpleft() returns the Stokes vector
    %     for light with left-turn circular polarization and intensity 1.
    %
    %     V = stokes_cpleft(p) returns the Stokes vector
    %     for light with left-turn circular polarization and intensity p.
    %
    %     A = stokes_cpleft(P), where P is a m x n x ... array of
    ...
    %
    %  See also:
    %     stokes_cpright stokes_lpvertical
    %
    %  File information:
    ...
    if (numel(intensity) > 1) || was_cell
        ...
        for Vi=1:numel(V)
            ...
            V{V_subs{:}} = s_cpleft(intensity(V_subs{:}));
        end
        
    else
        
        V = s_cpleft(intensity);
