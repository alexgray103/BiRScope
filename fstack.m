function [edofimg, varargout] = fstack(img_tmp,varargin)
...
filtersize = checkparam('logsize', 7);
...
for ii = 1:length(img)    
    ...
    fprintf('\b\b\b\b\b[%2.0i%%]',round(100*ii/length(img)))
end
...
for ii = 1:length(img)-1
    ...
    fprintf('\b\b\b\b\b[%2.0i%%]',round(100*ii/(length(img)-1)))
end
fprintf('\nFinished Focus stacking\n')
if nargout >= 2
    varargout{1} = fmap;
end
