...
% store stack into seperate arrays for later calculations
if length(size(imlist)) > 3
    P = size(imlist,4);
    imagesR = reshape(imlist(:,:,1,:), [size(imlist,1),size(imlist,2),size(imlist,4)]);
    imagesG = reshape(imlist(:,:,2,:), [size(imlist,1),size(imlist,2),size(imlist,4)]);
    imagesB = reshape(imlist(:,:,3,:), [size(imlist,1),size(imlist,2),size(imlist,4)]);
else
    P = size(imlist,3);
    imagesG = imlist;
end
...
if opts.RGB
    ...
    im = double(sum((imagesG.*fm), 3)./fmn);
end
fprintf('[100%%]\n')
t4 = toc;
...
options.RGB = (length(size(imlist))>3);
options.size = [M, N, P];
