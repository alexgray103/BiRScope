            ...
            if ~h.initialized
                ...
                switch(h.prefix)
                    ...
                    case Thorlabs.MotionControl.Benchtop.BrushlessMotorCLI.BenchtopBrushlessMotor.DevicePrefix103
                        ...
                        h.deviceNET = BenchtopBrushlessMotor.CreateBenchtopBrushlessMotor(serialNo);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    methods
        ...
        function set.isconnected(h, val)
            if val == 1
                assert(0, 'You cannot set the IsConnected property to 1 directly - please use connect(''serialnumber'') function!')        
            elseif val == 0 && h.isconnected
                h.disconnect;
            else
                assert(0, 'Unexpected value, could be only set to 0!')          
            end
        end
        ...
        function loaddlls() % Load DLLs
            if ~exist(motor.INTEGSTEPCLASSNAME,'class')
                try   % Load in DLLs if not already loaded
                    ...
                    errordlg('Unable to load .NET assemblies', 'NET assemblies Error')
                end
            end    
        end 
