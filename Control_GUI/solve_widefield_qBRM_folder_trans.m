function solve_widefield_qBRM_folder_trans(folder_loc, varargin)
...
%         end
        ...
        if length(size(img)) > 3
            ...
            [I0, phi, A, RGB_norm] = analytical_qBRM_gpu_all(img);
...
%             end
            ...
            write_zstack(uint8((255/3).*sum(ff_images,3)/max(I0(:))), sprintf('trans_background_%03d.tif',i))
            write_zstack(uint8(255*I0/max(I0(:))), sprintf('transmittance_%03d.tif',i))
            trans_scale_factor = 1/2;
           
            trans_corr=uint8(trans_scale_factor.*mean2((255/3)*sum(ff_images,3)).*I0./(max(I0(:))*(1/3).*sum(ff_images,3)));
            write_zstack(trans_corr, sprintf('transmittance_corrected_%03d.tif',i))
...
%             figure(),imshow(uint8(255*I0/max(I0(:))))
%             figure(),imagesc(uint8(I0/max(I0(:))))
            trans_corr=uint8(trans_scale_factor.*mean2((255/3)*sum(ff_images,3)).*I0./(max(I0(:))*(1/3).*sum(ff_images,3)));
            write_zstack(trans_corr, sprintf('transmittance_corrected_%03d.tif',i))
%             write_zstack(uint8((1/3).*sum(ff_images,3)/1000), sprintf('trans_background_%03d.tif',i))
%             write_zstack(uint8(I0/1000), sprintf('transmittance_%03d.tif',i))
...
%            
%             disp(trans_scale_factor)
%             trans_corr=uint8(trans_scale_factor.*mean2((1/3)*sum(ff_images,3)).*I0./((1/3).*sum(ff_images,3)));
%             write_zstack(trans_corr, sprintf('transmittance_corrected_%03d.tif',i))
            thresh=55;
            ...
            transweighted=transw.*(uint8(255*RGB_norm)+1);
%             size(RGB_norm)
%             size(transw)
%             size(transweighted)
           
            imwrite(transweighted,sprintf('trans_highlighted_%03d.tif',i))
